...
module Replicate =
    ...
    let getPrimaryKeyFromRow table (row: System.Data.DataRow) = 
        ...
        RTable(schema, tableN) ->
            ...
            RKey(table, cols 
                        |> Seq.map(fun col -> 
                                RColumn(table, col.column_name |> Option.defaultValue "")
                                ,     row.Item(col.column_name |> Option.defaultValue "") |> unbox<System.IComparable>)
                        |> Map.ofSeq)
        ...
        |> Seq.choose(function 
            (RTable(schemaP, nameP) as tableP, relId)->
                ...
                RTable(schema, name) ->
                    ...
                    if columns |> Seq.exists(fun col -> row.Value.Item(col.child_column  |> Option.defaultValue "") = box System.DBNull.Value) 
                    then None
                    else Some(
                              RKey(tableP, columns 
                                           |> Seq.map(fun col -> 
                                                let colP = col.parent_column |> Option.defaultValue ""
                                                let colC = col.child_column  |> Option.defaultValue ""
        ...
        Rop.flow {
            ...
            let! r =
                ...
                |> Seq.map(fun attV ->
                    ...
                    ) |> Option.defaultValue (Rop.succeed ())
                )
                |> Rop.seqCheck
            ()
        }
