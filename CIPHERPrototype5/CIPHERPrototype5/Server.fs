...
module Server =
    ...
    let private getTableDefinitions (user:User) dc = 
        let dimensionTables=
                                     ...
                                     |> Array.map(fun elem ->
                                        ...
                                        ,attributes
                                         |> Array.map(fun att ->
                                            ...
                                            |> Option.map(fun v -> v.value_string |> Option.defaultValue "") |> Option.defaultValue ""
                                            |> box
                                         )
                                      )
            ...
            Rop.flow {
                ...
                let  rels2            = rels
                                        |> Seq.map    (fun rel -> 
                                            ...
                                            child.element_name, parent.element_name, rel.weight |> Option.defaultValue 1.0, rel.child_order |> Option.defaultValue (child.element_order |> Option.defaultValue child.element_name))
                ...
                let getDes des name   = des |> Option.defaultValue name
                ...
                let putVal elem (aField: AttFieldOlap) v len = 
                                    v |> Option.map (fun v -> aField.putValueR_ dc (left len v) elem)
                                      |> Option.defaultValue (true |> Rop.succeed)
                let! res              = elements2
                                        |> Seq.map (fun elem ->
