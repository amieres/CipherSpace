...
//    let designCS = """Data Source=192.168.195.26;Initial Catalog=CIPHERSpaceDB;User ID=CIPHERSpaceUser;Password=cipher"""
    ...
    type User with
        ...
        member         this.name_             dc        = this.nameO_       dc |> Option.defaultValue "----"
        member         this.language_         dc        = this.languageRO_  dc |> Rop.ifError  None |> Option.defaultValue Language.defaultL
        member         this.errorUser_        dc        = this.userRecordR_ dc |> function | Success _ -> None | Failure m -> Some m
        member         this.themeTagsR_       dc        = this.userRecordR_ dc |> Rop.mapR(fun record -> record.theme_tags |> Option.defaultValue "")
    ...
    type User with
        ...
        member         this.clientsR_ (dc)         = this.userClientsRecordR dc 
                                                   |> Rop.mapR (Array.choose (fun record ->
        ...
        member         this.currentClientR_ (dc) = this.clientsR_ dc
                                                   |> Rop.bindR 
                                                      (fun clients ->
                                                         if clients.Length > 0 
    ...
    type Report with
        ...
        member         this.optionsR_  dc = this.reportDefinitionR_ dc |> Rop.mapR  (fun record -> record.options |> Option.defaultValue ""   )
    ...
    type ObjectT with
        ...
        member         this.nameR_           dc client = this.objectRecordR_ dc client |> Rop.mapR    (fun record -> record.object_name |> Option.defaultValue "")
...
//        member         this.addElemElemR_ child parent = Rop.liftTry "addElemElem"     (fun _ -> u (new addElemElem (c)) (fun cmd -> cmd.Execute(this.dimensionId, child, parent))) (Rop.succeed ())
        ...
        member         this.loadDataR_         ()      = 
            ...
            <| fun table -> 
                let rows =
                    table.Rows
                    |> Seq.sortBy (fun row -> row.element_order |> Option.defaultValue row.element_name)
                    |> Seq.map (fun element ->
                        element.ItemArray
                    )
                    |> Seq.toArray
...
//                        else sprintf "%d records updated." nTotal 
    ...
    let matchedTags (tags: string) (matchTo: string) =
        ...
        tags.Split([|' '|])
        |> Array.map(fun t ->
            if ts |> Array.tryFind (fun e -> e = t) |> Option.isSome
            then (-1, 0)
            else (-1, 1)
        )
            ...
            Rop.flow {
                ...
                let  res          = records 
                                    |> Seq.sortBy(fun record ->
                                          record.theme_tags |> Option.defaultValue "" |> matchedTags tags 
                                        , record.client_code = System.Guid.Empty)
                                    |> Seq.head
//                let  content = res.content_type |> Option.defaultValue "text/plain"
                let  lastModified = res.update_timestamp |> Option.defaultValue (res.create_timestamp |> Option.defaultValue (System.DateTime()))
