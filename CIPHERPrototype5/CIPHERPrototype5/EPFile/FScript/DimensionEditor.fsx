...
module DimTree =
    ...
    let view (props: Props) (model: Model) (processMessages: Message -> unit) =
        ...
        let addMoreFields idO rowSelected fields (tag: CipherNode list -> CipherNode) =
            ...
            |> List.mapi (fun i (v, setV) ->
                  ...
                  let editor, width, height, handleKey, focus =
                      ...
                      | true , Some (x, y, width, height, vO, _) -> true , width, height, (handleKeyEdit setV  ), vO |> Option.map (fun v -> (fun e -> e?value <- v; focus e; Some (x, y, width, height, None, Some (fun () -> e?value |> setV ; closeEditor processMessages)) |> SetEditor |> processMessages)) |> Option.defaultValue focus
        ...
        let FAtt i v = FAttribute(i, v |> Option.defaultValue "")
        let elemView priorRowSelected (elem: ElemEntry, expanded, symbol, level) =
            let fields = [ elem.name                                  , setV elem.id (Option.defaultValue "" >> FCode) ; 
                          (elem.description |> Option.defaultValue "")    , setV elem.id FDescription                  ;
                          (elem.order       |> Option.defaultValue "")    , setV elem.id FOrder                        ]
