...
module MenuBar =
            ...
            let rec menuEntry eid              = getElement eid |> Option.map(fun (_, id, name, desc, order, attribs:obj[]) ->
                                                    {
                                                        id          = eid
                                                        name        = name
                                                        description = desc  |> Option.filter (fun s -> s.Trim() <> "") |> Option.defaultValue name
                                                        order       = order |> Option.filter (fun s -> s.Trim() <> "") |> Option.defaultValue name
                                                        ...
                                                        children    = getChildren eid |> Seq.sortBy (fun (c, o) -> Option.defaultValue c.order o) |> Seq.map fst |> Seq.toList
                                                    }
                                                 )
    ...
    let menu (model: Model) name classes = 
        let entries = model.entries |> Seq.choose (fun e -> if e.name = name then Some e.children else None) |> Seq.tryHead |> Option.defaultValue []
        let rec dropDown level (e:MenuEntry) =
            Li [ 
                 A [ 
                     ...
                     match e.content with | Some content when content.Trim() <> "" -> yield Href ("/EPItem/" + e.name) | _ -> ()
                   ]
                 |> addChildren
