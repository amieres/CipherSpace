...
module TreeElement =
    ...
    let view (props: Props) (model: Model) (processMessages: Message -> unit) =
        ...
        let layout = props.layout |> Option.defaultValue Div
...
module TreeElements =
    ...
    type TreeEntry<'T>  = {
                            id          : RowId
                            name        : string
                            children    : TreeEntry<'T> seq
                            layout      : (CipherNode * bool * bool * int * (unit -> unit) -> CipherNode) option
                            other       : 'T
                          }             
    ...
    let node props : CipherNode =
        let layout       = props.layout       |> Option.defaultValue Div
        let elemView (elem: TreeDetail<_>) =
            let entryLayout = elem.entry.layout |> Option.defaultValue (fun (content, hasChildren, expanded, level, onClick) -> TreeElement.next content hasChildren expanded level onClick None)
...
module TreeView =
    ...
    type TreeEntry<'T>   = TreeElements.TreeEntry<'T>
    type TreeDetail<'T>  = TreeElements.TreeDetail<'T>

    let rec flat (elements: TreeEntry<_> seq) =
    ...
    type NextNode<'T> = TreeDetail<'T> seq * (TreeElements.RowId -> bool -> unit) * (unit -> unit) 
                                                                                  * (unit -> unit) 
                                                                                  * (unit -> unit) 
                                                                                  * (unit -> unit) 

    type Props<'T> = { elements : TreeEntry<'T> seq 
                       layout   : (NextNode<'T> -> CipherNode) option
                     }
    ...
    let update (props: Props<_>) (msg: Message)  model = 
        match msg with 
    ...
    let view (props: Props<_>) (model: Model) (processMessages: Message -> unit) =
        let setExpanded id exp   = (id, exp) |> SetExpanded |> processMessages
        let elementsSet elements = elements |> Seq.map (fun (elem: TreeEntry<_>) -> elem.id) |> Set
        ...
        let rec elemRows level (elems: TreeEntry<_> seq) =
            elems
        ...
        let layout = props.layout |> Option.defaultValue (fun (elements, setExpanded, expandOne, collapseOne, expandAll, collapseAll) -> 
                                                               TreeElements.next elements setExpanded None)
        (elements, setExpanded, expandOne, collapseOne, expandAll, collapseAll) |> layout 

    let app<'T> = App.App<Props<'T>,Model, Message>(init, update, view)
...
module GridCell =
    ...
    type DCellText  = DCellText  of DataTube.Cell<string   , DataIdX, DataIdX>
      with 
        ...
        member this.getData               = match this with DCellText cell -> cell.getDataO() |> Option.defaultValue ""
    ...
    and  DCellState = DCellState of DataTube.Cell<CellState, DataIdX, DataIdX>
      with 
        ...
        member this.getData               = match this with DCellState cell -> cell.getDataO() |> Option.defaultValue Idle
    ...
    let view (props: Props) (model: Model) (processMessages: Message -> unit) =
        ...
        let afterRender : (obj -> unit) option =
            match cellState with
                 | Editing (width, height, vO) when model.initEdit -> Some (fun e -> e?value <- vO |> Option.defaultValue text ; props.editNotify |> Option.iterF e ; SetInitEdit false |> processMessages)
                 | Current      
...
module GridRows =
    ...
    type GridEntry<'T> = {
                           id           : TreeElements.RowId
                           fields       : GridCell.DCellText seq
                           isInsertion  : bool
                           other        : 'T
                         }
    ...
    type Props<'T>     = { 
                           ...
                           elements     : RowDetail<'T> seq
                           rowLayout    : (CipherNode seq                  -> CipherNode) option
                           layout       : (CipherNode seq * CipherNode seq -> CipherNode) option
                         }

    let node props : CipherNode =
        let rowLayout    =  props.rowLayout  |> Option.defaultValue Tr
        let layout       =  props.layout     |> Option.defaultValue (fun (h, es) -> Table [ THead [ Tr h ] ; TBody es])
        ( props.header
...
module GridTable =
    ...
    type GridEntry<'T> = GridRows.GridEntry<'T>

    type NextStage<'T> = {
                        header          : CipherNode seq
                        rows            : GridRows.RowDetail<'T> seq
                        selectedRows    : (GridEntry<'T> seq -> unit) -> unit
                        model           : Model
                        props           : Props<'T>
    ...
    and  Props<'T> = {
                       elements         : GridEntry<'T> seq 
                       columns          : string seq 
                       cellLayout       : (int * CipherNode seq -> CipherNode) option
                       layout           : (NextStage<'T>        -> CipherNode) option
                       addElements      : int -> (GridEntry<'T> seq -> unit) -> unit
                     }                  
    ...
    let scanFields model f (elem: GridEntry<_>) rowSelected =
        elem.fields 
        |> Seq.mapi (fun i text -> let state = cellState model (Some elem.id) rowSelected (ColId i)
                                   f elem.id i text state
                    )

    let scanRow (model: Model) (f: GridEntry<_> -> bool -> 'a) priorRowSelected (elem: GridEntry<_>) =
    ...
    let scanRows (elements: GridEntry<_> seq) model f =           elements          |> Seq.mapFold (scanRow    model f) false |> fst
    ...
    let update processMessages (props: Props<_>) (msg: Message)  model =
    ...
    let pasteRegion (props: Props<_>) (model: Model) (nRows, nCols) (maxRows, maxCols) =
    ...
    let pasteText (props: Props<_>) (model: Model) (text:string) =
    ...
    let copyText  (elements:GridEntry<_> seq)  model =
        elements
    ...
    let handleKey cellId (text:GridCell.DCellText) (props: Props<_>) (processMessages: Message -> unit) ev (model: Model) =
        let getCurrentId1 =
            match model.selection with
 ...
 //; Some (x, y, width, height, None, Some (fun () -> setVO |> Option.iterF e?value ; closeEditor processMessages)) |> SetEditor |> processMessages))
    ...
    let getSelectedRows (props: Props<_>) f (model: Model) =
        scanRows props.elements model (fun entry selected -> if selected then Some entry else None)
        |> Seq.choose id
        |> f

    let view (props: Props<_>) (model: Model) (processMessages: Message -> unit) =
        let cellLayout      = props.cellLayout |> Option.defaultValue (fun (i,s) -> Td s)
        let layout          = props.layout   |> Option.defaultValue (fun {header = header; rows = rows} -> GridRows.simpleNode header rows)
        ...
        let renderRow (elem: GridRows.GridEntry<_>) (isRowSelected:bool) =
...
module DimElements =
    ...
    type ElemEntry (idP          : TreeElements.RowId
        ...
        do _description.setOnly (descriptionP |> Option.defaultValue "")
        do _order.setOnly       (orderP       |> Option.defaultValue "")
        member this.id            = idP
        ...
        member this.changed       = _name.getData        <> nameP 
                                 || _description.getData <> (descriptionP |> Option.defaultValue "")
                                 || _order.getData       <> (orderP       |> Option.defaultValue "")
    ...
    let SimpleTreeEntry(_id, _name, _children, _layout) = {
        TreeView.TreeEntry.id          = _id      
        TreeView.TreeEntry.name        = _name    
        TreeView.TreeEntry.children    = _children
        TreeView.TreeEntry.layout      = _layout
        TreeView.TreeEntry.other       = ()
    }

    let rec translateEntry (elem: EntryHier) : TreeView.TreeEntry<_> =
    ...
    (*let rec translateTableEntry (elem: ElemEntry) : TreeView.ITreeEntry =
        let fields es = seq[ yield  Seq.append es [ elem.name |> TreeElements.text       ] |> Td
                             yield  [ elem.description |> Option.defaultValue ""    |> NText ] |> Td
                             yield  [ elem.order       |> Option.defaultValue ""    |> NText ] |> Td
    ...
    let addNewElements (props:Props) processMessages n f model =
        let entryPoint = props.addPoint |> Option.defaultValue Append
        let newElements = [1..n] 
    ...
    let loadData_ processMessages (props: Props) =
        ARop.wrap {
            ...
            let  elems                         = elements 
                                                 |> Seq.sortBy (fun (_, id, name, desc, order, attribs:obj[]) -> order |> Option.filter (fun s -> s.Trim() <> "") |> Option.defaultValue name)
    ...
    let getRelations (model :Model) =
        let getName eid = eid |> getElement model |> Option.map(fun e -> e.name.getData) |> Option.defaultValue ""
    ...
    let view (props: Props) (model: Model) (processMessages: Message -> unit) =
        let layout = props.layout |> Option.defaultValue (treeViewNext None)
        let emptyNext_ wait = { 
                elements        = []
                columns         = []
                props           = props
...
module DimensionListAll =
    ...
    let view (props: Props) (model: Model) (processMessages: Message -> unit) =
        ...
        let layout = props.layout |> Option.defaultValue defLayout
    ...
    let selTuple (cell: DataTube.Cell<DimTuple, string, string>) =  
        ...
        | Some (dim, did, name, descO) -> (Some dim, Some did, name, descO |> Option.defaultValue "")
        | None                         -> (None    , None    , ""  ,                          "")
...
module GridArray =
    ...
    type GridEntry     = GridTable.GridEntry<unit>
    ...
    type NextStage       = {
                             ...
                             processMessages : Message -> unit
                             rows            : GridEntry seq 
                             addElements     : int -> (GridEntry seq -> unit) -> unit
                           }
    ...
    let addNewRows props model processMessages n f : unit =
        ...
        newRows |> Array.map (fun elem -> {
                                            GridEntry.id           = TreeElements.RowId (elem.id :?> int)
                                            GridEntry.fields       = row props elem
                                            GridEntry.isInsertion  = false
                                            GridEntry.other        = ()
                                          }
                             )
        |> Seq.ofArray
    ...
    let view (props: Props) (model: Model) (processMessages: Message -> unit) =
        ...
        let gridElements =
            model.rows 
            |> Array.mapi(fun i r ->            {
                                                  GridEntry.id           = TreeElements.RowId i
                                                  GridEntry.fields       = row props r
                                                  GridEntry.isInsertion  = false
                                                  GridEntry.other        = ()
                                                })
            |> if not props.append then id
               else (fun a -> Array.append a [| {
                                                  GridEntry.id           = TreeElements.RowId model.rows.Length
                                                  GridEntry.fields       = entryNew
                                                  GridEntry.isInsertion  = true
                                                  GridEntry.other        = ()
                                                } |])
        let layout = props.layout |> Option.defaultValue (fun nextStage -> GridTable.next nextStage.rows nextStage.props.columns nextStage.addElements None None )
        {
          props           = props
          model           = model
          processMessages = processMessages
...
module DimEditor =
    ...
    let GridTreeEntry(_id, _children, _layout, _fields, _insertion) =
        { 
          TreeView.TreeEntry.id          = _id  
          TreeView.TreeEntry.name        = ""
          TreeView.TreeEntry.children    = _children
          TreeView.TreeEntry.layout      = _layout
          TreeView.TreeEntry.other       = {
                                              GridTable.GridEntry.id          = _id      
                                              GridTable.GridEntry.fields      = _fields
                                              GridTable.GridEntry.isInsertion = _insertion
                                              GridTable.GridEntry.other       = ()
                                            }
        }

    let TreeDetailGridEntry(i:int, detail:TreeElements.TreeDetail<GridTable.GridEntry<unit>>) =
        { 
          GridTable.GridEntry.id          = TreeElements.RowId i      
          GridTable.GridEntry.fields      = detail.entry.other.fields
          GridTable.GridEntry.isInsertion = detail.entry.other.isInsertion
          GridTable.GridEntry.other       = detail
        }

    let GridTreeEntry2(_entry: TreeView.TreeEntry<_>,  layout) =
        { _entry with layout      = layout }

    let rec translateGridEntry (elem: DimElements.EntryHier) : TreeView.TreeEntry<GridRows.GridEntry<unit>> =
    ...
    let gridRowToTreeEntry2 addRelation (elem:GridRows.RowDetail<TreeElements.TreeDetail<_>>) =
        let detail = elem.entry.other
    ...
    let update (props: Props) (msg: Message)  (model: Model) : Model = 
        match msg with 
        | SetModified    m  -> JS.Window.Onbeforeunload <- System.Action<Dom.Event>(
                                                               match m with
    ...
    let renderCallback fO p : CipherNode = fO |> Option. map (fun f -> f p) |> Option.defaultValue NEmpty
    ...
    let DimTree (dimElemsNS: DimElements.NextStage) elements fO =
        render {
            ...
            let  translateTreeDetails           = Seq.mapi (fun i elem -> (i,elem) |> TreeDetailGridEntry)
            let  gridItems                      = treeArray |> translateTreeDetails |> Seq.toList 
            let  addElements                    = fun n f -> dimElemsNS.addElements n ( Seq.map translateGridEntry >> Seq.cast<GridTable.GridEntry<_>> >> f)
            let! { header       = header
                   rows         = rows
        ...
        render {
            ...
            return Layouts.leftContentRightNode
                       ...
                       [ Layouts.bootstrapPanel
                             [ 
                               Label [ Class "panel-title text-center"
                                       model.selected |> Option.map (snd >> fun n -> if dimElemsNS.model.modified then n + " *" else n) |> Option.defaultValue "<-- Select a dimension..." |> NText
                                       dimElemsNS.wait
