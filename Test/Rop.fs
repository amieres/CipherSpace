...
//      member this.Bind<'a, 'b> (wrapped: Async<'a>        , restOfCExpr: 'a -> Wrapper<'b>) = wrapped |> WAsync  |> bind restOfCExpr  << cannot differentiate from prior
    ...
    let call (wb: Wrapper<unit>)=
        match wb with
        | WSimple   _ -> ()
        | WResult  rb -> rb  |> Rop.notifyMessages
        | WAsync   ab -> Async.StartWithContinuations(ab , (fun _ -> ())     , (fun exc -> Rop.failException exc |> Rop.notifyMessages), fun can -> Rop.failException can |> Rop.notifyMessages)
        | WAsyncR arb -> Async.StartWithContinuations(arb, Rop.notifyMessages, (fun exc -> Rop.failException exc |> Rop.notifyMessages), fun can -> Rop.failException can |> Rop.notifyMessages)
