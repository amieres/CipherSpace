...
module Repository =
    ...
    [<Literal>]
    let designCS = """Data Source=192.168.195.44;Initial Catalog=CIPHERSpaceDB;Integrated Security=False;User ID=Abelardo.Mieres;pwd=Secure4P@ss;"""
    ...
    type private SqlClientParameters = SqlCommandProvider<"ClientParameters.sql", designCS>
    ...
    type SqlAccounts        = SqlCommandProvider<"SELECT ID, IsNull(Name, '') Name, IsNull(ALRE, '') ALRE, IsNull(OrderPosition, 0) OrderPosition 
                                                FROM tblAccounts", EnumdesignCSs.``Designer Connection String``, ResultType.DataTable>
    let getAccountsR (user: User) dc () =
        Rop.flow {
            let! client  = user.currentClientR_ dc
            let! connStr = client.designerConnStrR_ dc
            use  cmd     = new SqlAccounts(connStr)
    ...
    type SqlCompanies        = SqlCommandProvider<"SELECT ID, Name, OrderPosition FROM tblLegalEntities", EnumdesignCSs.``Designer Connection String``, ResultType.DataTable>
    let getCompaniesR (user: User) dc () =
        Rop.flow {
            let! client  = user.currentClientR_ dc
            let! connStr = client.designerConnStrR_ dc
            use  cmd     = new SqlCompanies(connStr)
            let  table   = cmd.Execute()
            let  data    =
                    table.Rows
                    |> Seq.map (fun row -> row.ItemArray)
    ...
    type SqlClientParametersTable = SqlCommandProvider<"ClientParametersTable.sql", designCS, ResultType.DataTable>
    // test: getClientParametersTable "Genesis" "chinese"
    let getClientParametersTableR (user: User) (dc: DataCache) () =
        Rop.flow {
            do! Rop.tryProtection()
            let! client      = user.currentClientR_ dc
            let  language    = user.language_       dc
            use  cmd         = new SqlClientParametersTable(designCS)
            let  table       = cmd.Execute(language.languageCode, client.clientCode)
            let  data        =
                    table.Rows
                    |> Seq.map (fun row -> row.ItemArray)
                    |> Seq.toArray
    ...
    type ObjectHierarchy = XmlProvider<"SampleObjectHierarchy2.Xml", Global=true>
    ...
    let addSensitive (table: System.Data.DataTable) c =
        let primary = Seq.exists (fun (f:System.Data.DataColumn) -> f.ColumnName = c.ColumnName) (table.PrimaryKey)
        { c with Sensitive = c.ColumnName.EndsWith "_password" || c.DataTypeName = "Binary" || c.DataTypeName = "Guid" || primary}

    let  columnsFromTable (table: System.Data.DataTable) =
        let columns =
            table.Columns
            |> Seq.cast<System.Data.DataColumn>
            |> Seq.map (fun c ->
                { ColumnName     = c.ColumnName
                  Caption        = if   c.ExtendedProperties.ContainsKey "MS_Description" 
                                   then c.ExtendedProperties.["MS_Description"] :?> string |> Some 
                                   else None
                  MaxLength      = c.MaxLength
                  AllowDBNull    = c.AllowDBNull
                  DataTypeName   = c.DataType.Name
                  Sensitive      = false
                }
                |> addSensitive table
            )
    ...
    let processRowValues (columns: ColumnDefinition list) (row: obj[]) = 
        Seq.zip columns row
        |> Seq.choose (fun (column, v) ->
            if column.Sensitive then None
            else 
                match v with
                    | :? System.DBNull -> DNull
                    | _                -> 
                        match column.DataTypeName with
                            | "String" -> DText  (unbox     v)
                            | "Int"    -> DInt   (unbox     v)
                            | _        -> DUnsupported
                |> Some
        )
    ...
    let private fetchTableSchema designCS query =
        ...
        conn.ConnectionString <- designCS
        conn.Open()
    ...
    let  getSingleRecordData (title:string) (table: System.Data.DataTable) =
        Rop.flow {
            do! Rop.tryProtection()
            do! (table.Rows.Count = 1) |> Rop.assertR <| ErrRecordNotFound (title, "")
            let columnsAll = columnsFromTable table
            let data       =  table.Rows.Item(0).ItemArray  |> processRowValues columnsAll
            let columns    = columnsAll |> List.filter (fun c -> not c.Sensitive)
            return title, columns, data
        }

//    let updateSingleRecordData (designCS:string) (data: DataValue array) (tableName:string) (condition:string) (parms: seq<System.Data.SqlClient.SqlParameter>) (querySelect: string) =
//        let fieldsAndParms concat fields =
//            fields  |> Seq.map (fun (c:ColumnDefinition) -> c.ColumnName + " = @" + c.ColumnName) |> String.concat concat
////        let fields =
////            columns |> String.concat ", "
////        let parms =
////            columns |> Seq.map ((+)"@") |> String.concat ", "
////        let condition  =
////            keys 
////            |> fieldsAndParms " AND "
//        let columnsSet columns =
//            columns 
//            |> fieldsAndParms ", "
//        let sqlParm (schema:System.Data.DataTable) (column:ColumnDefinition) =
//            schema.Rows 
//            |> Seq.cast<System.Data.DataRow> 
//            |> Seq.find(fun row -> row.["ColumnName"] = (column.ColumnName :> obj))
//            |> fun c -> ("@" + column.ColumnName, c.["ProviderType"] :?> System.Data.SqlDbType, c.["ColumnSize"] :?> int)
//        let addParameters (cm : System.Data.SqlClient.SqlCommand) schema (fields : seq<ColumnDefinition>) =
//            fields 
//            |> Seq.map (sqlParm schema)
//            |> Seq.iter(cm.Parameters.Add >> ignore)
//        Rop.flow {
//            do! Rop.tryProtection()
//            use schema      = fetchTableSchema designCS querySelect
//            let columns     = 
//                schema.Rows 
//                |> Seq.cast<System.Data.DataRow> 
//                |> Seq.map(fun row -> 
...
//                          Caption        =  None
//                          MaxLength      =  0 //row.["MaxLength"  ] :?> int
//                          AllowDBNull    =  row.["AllowDBNull"] :?> bool
//                          DataTypeName   = (row.["DataType"   ] :?> System.Type).Name
//                          Sensitive      =  false
//                        }
//                        |> addSensitive table
//                    )
//                    |> Seq.filter (fun c -> not c.Sensitive)
//            use  conn  = new System.Data.SqlClient.SqlConnection()
//            conn.designCS <- designCS
//            conn.Open()
//            use cmdUpd = new System.Data.SqlClient.SqlCommand("UPDATE " + tableName + " SET " + (columnsSet columns) + " WHERE " + condition, conn)
//            columns |> addParameters cmdUpd schema
//            data    |> Seq.iteri (fun i dv -> cmdUpd.Parameters.[i].Value <-
//                                                match dv with
//                                                | DText  s  -> s                   :> obj
//                                                | DFloat f  -> f                   :> obj
//                                                | DInt   i  -> i                   :> obj
//                                                | _         -> System.DBNull.Value :> obj
//                    )
//            parms |> Seq.iter (cmdUpd.Parameters.Add >> ignore)
//            let  n      = cmdUpd.ExecuteNonQuery()
//            do! (n = 1) |> Rop.assertR <| ErrRecordNotFound (tableName, cmdUpd.CommandText) 
//            ()
//        }        
//
    let updateSingleRecordData2 (designCS:string) (data: DataStorage) (table: System.Data.DataTable) (update: unit -> int) =
        Rop.flow {
            do! Rop.tryProtection()
            let! data = match data with
                        | SingleRow data -> Rop.succeed data
                        | _              -> Rop.fail ErrUnsuportedDataStorage
            do! (table.Rows.Count = 1) |> Rop.assertR <| ErrRecordNotFound ("updateSingleRecordData2", "")
            let columnsAll = columnsFromTable table
            let columns    = columnsAll |> List.filter (fun c -> not c.Sensitive)
            data |> Seq.zip columns |> Seq.iter (fun (c, dv) -> table.Rows.[0].Item(c.ColumnName) <-
                                                                                            match dv with
                                                                                            | DText  s  -> s                   :> obj
                                                                                            | DFloat f  -> f                   :> obj
                                                                                            | DInt   i  -> i                   :> obj
            ...
            do! (update() = 1) |> Rop.assertR <| ErrRecordNotFound ("Update", "") 
        }
    ...
    type private QIFUser      = SqlCommandProvider<"SELECT *              FROM CS.[User]           WHERE user_code      = @userCode      ", designCS, ResultType = ResultType.DataReader>
    type private QIFClient    = SqlCommandProvider<"SELECT *              FROM CS.Client           WHERE client_code    = @clientCode    ", designCS, ResultType = ResultType.DataReader>
    ...
    let tblQIFUser      uC f    = rT (fun () -> u (new QIFUser        (c)) (fun cmd -> u (cmd.Execute uC    ) (fun rdr -> u (new DB.CS.Tables.User            ()) (fun tbl -> tbl.Load(rdr); f tbl))))
    let tblQIFClient    cC f    = rT (fun () -> u (new QIFClient      (c)) (fun cmd -> u (cmd.Execute cC    ) (fun rdr -> u (new DB.CS.Tables.Client          ()) (fun tbl -> tbl.Load(rdr); f tbl))))
    ...
    type InputFormId with
        member this.fetchSingleRecordDataR(user: User, client: Client) =
            match this with
            | IFUser                 -> tblQIFUser      user.userCode         <| getSingleRecordData "User settings"
            | IFClient               -> tblQIFClient    client.clientCode     <| getSingleRecordData "Company settings"
            | IFDimension(dimension) -> tblQIFDimension dimension.dimensionId <| getSingleRecordData "Dimension settings"
        member this.updateSingleRecordDataR(user: User, client: Client) (data: DataStorage) =
            let updateSingleRecordData3 tbl update = updateSingleRecordData2 designCS data tbl update
            match this with
            | IFUser                 -> tblQIFUser      user.userCode         <| fun tbl -> updateSingleRecordData3 tbl tbl.Update
            | IFClient               -> tblQIFClient    client.clientCode     <| fun tbl -> updateSingleRecordData3 tbl tbl.Update
            | IFDimension(dimension) -> tblQIFDimension dimension.dimensionId <| fun tbl -> updateSingleRecordData3 tbl tbl.Update
...
//        member         this.addElemElemR_ child parent = Rop.liftTry "addElemElem"     (fun _ -> u (new addElemElem (c)) (fun cmd -> cmd.Execute(this.dimensionId, child, parent))) (Rop.succeed ())
        ...
        member         this.saveRelsDataR_  (deleted : obj [][]) (changed : obj [][]) (added : obj [][]) (relations: (string * string) []) =
            ...
            <| fun table -> 
                Rop.flow {
                    ...
                    let saveElementsData =
                            ...
                            |> Array.map (fun row ->
                                ...
                                |> Array.iteri(fun i attribute ->
                                    if i+5 < row.Length then
                                        tableAtt.LoadDataRow([| box attribute.attribute_code; newId ; getAttV attribute row.[i + 5] |], System.Data.LoadOption.Upsert) |> ignore
                                )
                                row.[0] <- box this.dimensionId  
                                row.[1] <-     newId
                                table.LoadDataRow(row.[0..4], System.Data.LoadOption.Upsert) |> ignore
                        ...
                        table.EndLoadData()
                        let nTotal = table.Update()
                        tableAtt.EndLoadData()
                        let nTotal2 = tableAtt.Update()
                        let nDifference = (deleted.Length + added.Length + changed.Length - nTotal)
                    ...
                    let deleted =
                        currents
                        |> Seq.filter(fun (child', parent', _, _) -> relations |> Array.exists(fun (child, parent) -> child' = child && parent' = parent) |> not)
