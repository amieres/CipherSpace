...
//        |> Seq.append assemblies
    ...
    let compileR_ (code:string) assemblies inMemory = 
        Rop.flow {
            ...
            let src       = System.IO.Path.ChangeExtension(fn, ".fs")
    ...
    let getJSR_ minified (context: Core.Resources.Context) (fsx:string) = 
        ...
        Rop.flow {
            ...
            js.AssemblyInfo.Requirements |> Seq.map(fun (a, b) -> a) |> js.Info.GetDependencies |> Seq.iter(fun r -> r.Render ctx (fun _ -> writer) )
