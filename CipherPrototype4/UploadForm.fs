...
module UploadForm =
    ...
    let getFileTime_ () = 
        Rop.flow {
            let! fileO = getUploadFileRO_()
            return fileO |> Option.map (fun f -> f.LastModifiedDate.ToUTCString()) |> Option.defaultV ""
        } |> Rop.ifError ""
    ...
    let view (props: Props) (model: Model) processMessages =
        ...
        let getFolderPath idO           = idO |> Option.bind getFolder |> Option.map path_ |> Option.defaultV ""
