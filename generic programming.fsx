...
let x() = sql "SELECT name, age" ++ sql " FROM Customers" 
let y() =  x() id
let z() = y() (QueryContext ("", 0, [], (fun counter -> sprintf "@p%d" counter), 
                              (fun paramName value -> new SqlParameter(paramName, value) :> _)))

z()

(``%d`` ++ ``%s``) id (QueryContext ("", 0, [], (fun counter -> sprintf "@p%d" counter), 
                              (fun paramName value -> new SqlParameter(paramName, value) :> _))) 88 "hola"


let inline doSomethingElse (x :  ^a) (y : ^b) =
    let z = x + y
    printfn "%A" z

let inline doThis (x :  ^a) = (^a : (static member hello: int -> string) (5))

type B = B with static member hello i = sprintf "HELLO %d" i
